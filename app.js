...
/* ══════════════════════════════════════════════════════════
   ✨ TEAMSYNC ANIMATIONS — Particles · Ripple · Scroll Reveal · Tilt
══════════════════════════════════════════════════════════ */
(function(){
  /* ── Floating Particles ── */
  ...
  if(canvas){
    ...
    let W, H, particles = [];
    const COLORS = ['rgba(192,132,252,', 'rgba(129,140,248,', 'rgba(244,114,182,', 'rgba(252,211,77,', 'rgba(110,231,183,'];
    ...
    function draw(){
      ...
      particles.forEach(p=>{
        p.x+=p.dx; p.y+=p.dy; p.life+=.003;
        if(p.y<-10||p.life>1){ p.y=H+5; p.x=Math.random()*W; p.life=0; }
  ...
  document.addEventListener('click', e=>{
    const btn = e.target.closest('.btn,.reg-btn');
    if(!btn) return;
  ...
  function addTilt(selector){
    document.querySelectorAll(selector).forEach(card=>{
      ...
      card.addEventListener('mousemove', e=>{
        ...
        card.style.transition='transform .08s ease,border-color .2s,box-shadow .2s';
        card.style.transform=`perspective(700px) rotateY(${x*6}deg) rotateX(${-y*5}deg) translateY(-4px) scale(1.01)`;
      });
      card.addEventListener('mouseleave', ()=>{
        ...
        setTimeout(()=>{ card.style.transition=''; card.style.transform=''; },360);
      });
    });
  }
  // Attach tilt to stat cards and hackathon cards
  ...
  attachTilts();
  // Re-attach after screens change (screens rendered dynamically)
  const _orig = window.showScreen;
  if(_orig){ window.showScreen = function(){ _orig.apply(this,arguments); setTimeout(attachTilts,300); }; }

  /* Scroll Reveal removed */

  /* ── LeetCode progress bar width from inline style ── */
  document.addEventListener('DOMContentLoaded',()=>{
    document.querySelectorAll('.lc-progress-fill').forEach(el=>{
  ...
  /* ── Nav click micro bounce ── */
  document.addEventListener('click', e=>{
    const btn=e.target.closest('.tn,.mn-btn,.mm-nav-item');
    if(!btn) return;
